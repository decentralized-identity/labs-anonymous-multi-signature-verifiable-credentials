...
export default function GroupSetup() {
  ...
  const [chainId, setChainId] = useState('eip155:11155111')
  const [merkleRootHistoryEndpoint, setMerkleRootHistoryEndpoint] = useState('')
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <h1 className="text-3xl font-bold mb-8">Semaphore Group Setup with Group DID</h1>
      ...
      {!groupInfo ? (
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium mb-1">Min Approvals (m)</label>
                <input
                  type="number"
                  value={m}
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Total Members (n)</label>
                <input
                  type="number"
                  value={n}
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Merkle Root History Endpoint (Optional)</label>
              <input
                type="text"
        ...
        <div className="space-y-6">
          <div className="bg-green-50 p-4 rounded-lg">
            <h2 className="text-lg font-semibold text-green-800 mb-2">Group Created Successfully!</h2>
            <div className="space-y-2 text-sm">
              <p><strong>Group DID:</strong> <code className="bg-white px-2 py-1 rounded">{groupInfo.did.did}</code></p>
              <p><strong>Group ID:</strong> {groupInfo.semaphoreGroup.config.groupId}</p>
              <p><strong>Merkle Root:</strong> <code className="text-xs break-all">{groupInfo.semaphoreGroup.merkleRoot}</code></p>
              <p><strong>Approval Policy:</strong> {groupInfo.semaphoreGroup.config.approvalPolicy.m} of {groupInfo.semaphoreGroup.config.approvalPolicy.n}</p>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium mb-1">Member Identity Secret</label>
            ...
            <div className="mt-6">
              <h4 className="font-medium mb-2">Current Members ({groupInfo.semaphoreGroup.memberCount})</h4>
              <div className="space-y-1">
                {groupInfo.semaphoreGroup.members.map((member, idx) => (
                  <div key={idx} className="text-xs bg-gray-50 p-2 rounded font-mono break-all">
                    {member}
                  </div>
                ))}
              </div>
